...
def out(data, right):
    ...
    f = input()
    if f.lower() != right:
        print('\033[1m\033[30m\033[41m{}'.format('Вы олег! Попробуйте еще раз!'))
        print('\033[0m\033[' + str(random.randint(31, 37)) + 'm{}'.format(data), end="\n> ")
        f = input()
        if f.lower() != right:
            logout()
...
def change(data):
    ...
    if f.isnumeric() and 0 < int(f) <= len(data):
        ...
        print('\033[1m\033[30m\033[41m{}'.format('Вы олег! Попробуйте еще раз!'))
        print(
            '\033[0m\033[' + str(random.randint(31, 37)) + 'm{}'.format('Для начала выберете наиболее интересную вас '
                                                                        'тему: '))
        a = 1
        for d in data:
            print('\033[' + str(random.randint(31, 37)) + 'm{}'.format(str(a) + ") " + d))
